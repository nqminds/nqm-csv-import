...
exports.import = (function() {
  ...
  Importer.prototype.start = function(config, cb) {
    ...
    self._bulkMode = self._config.bulkMode !== false;
    self._upsertMode = self._config.upsertMode !== false;
  ...
  var flushBulkQueue = function(cb) {
    ...
    if (self._bulkQueue.length > 0) {
      var bulkLoad = self._upsertMode ? helpers.upsertDatasetDataBulk : helpers.addDatasetDataBulk;
      bulkLoad(self._config.commandHost, self._accessToken, self._config.targetDataset.id, self._bulkQueue, function(err) {
        if (err) {
    ...
    if (self._queue.length > 0 && self._processCount < self._config.endLine) {
      ...
      if (!isEmpty && self._processCount > self._config.startLine) {
        ...
        if (self._bulkMode) {
          ...
          // Attempt to add the data to the dataset.
          var dataLoad = self._upsertMode ? helpers.upsertDatasetData : helpers.addDatasetData;
          dataLoad(self._config.commandHost, self._config.targetDataset.id, persist, function(err) {
            if (err) {
